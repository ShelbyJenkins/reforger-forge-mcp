...
| MCP-056 | Implementation complete; snapshot and live default-root validated | 2026-08-05 (snapshot), 2026-08-07 (default-root) | Focused recovery tests 9/9 and the complete serial suite 2,278/2,278 passed with one intentional skip. Snapshot acceptance classified retained history, refused foreign authority without mutation, and proved controlled idle exit. Landed in `8b15049`. MCP-057 (native crash on existing-only Workbench LMDB inspection under another live host) was resolved 2026-08-06. With Workbench and every other MCP host confirmed stopped, the true default-root gate was then re-run directly against `%LOCALAPPDATA%\ReforgerForge\Observer\v1`: real cross-installation history correctly refused recovery, and after the root was cleared the host committed clean idle shutdown in 60.5-63.6s with exit code 0 across three runs. That re-run also found and fixed `MCP-071`, an unrelated idle-shutdown seal race in `CaptureService`'s retention sweep. See the [adversarial review](2026-08-05-launch-ergonomics-adversarial-review.md#p3) and `agents/mcp-tracking/MCP_BUGS_RESOLVED.md`. |
