...
export function registerScenarioTools(server: McpServer, client: WorkbenchClient): void {
  ...
  // ---------------------------------------------------------------------------
  // Conflict base placement tool constants
  // ---------------------------------------------------------------------------
  ...
  server.registerTool(
    ...
    {
      description:
        "Create a scenario element in the live Workbench scene. " +
        "Use type='objective' for Scenario Framework objectives (SP/co-op narrative missions): " +
        "places an Area → LayerTask → Layer_AI → SlotKill + SlotAI hierarchy. " +
        "Use type='base' for Conflict multiplayer bases: places a ConflictBase entity, " +
        "ambient patrol spawnpoints, and a faction spawn point. " +
        "Requires Workbench running in Edit mode with a world open.",
      inputSchema: {
        type: z
          .enum(["base", "objective"])
          .describe("Action discriminator. 'objective' = Scenario Framework objective (SP/co-op). 'base' = Conflict military base (MP)."),

        // --- shared ---
        position: z
          .string()
          .optional()
          .describe("World position as 'x y z' (e.g. '1234 0 5678'). Omit to place at the current camera position."),

        // --- (objective) params ---
        taskType: z
          .enum(["kill", "clearArea", "destroy"])
          .optional()
          .describe("(objective) Objective type. Determines which LayerTask and Slot prefabs are used."),
        taskName: z
          .string()
          .optional()
          .describe(
            "(objective) Short identifier used as entity name prefix and task title (e.g. 'Eliminate_Patrol'). " +
            "No spaces — use underscores. All placed entity names derive from this."
          ),
        description: z
          .string()
          .optional()
          .describe("(objective) Task description shown to the player in the task list."),
        targetPrefab: z
          .string()
          .optional()
          .describe(
            "(objective) Prefab path for the entity the player must kill/destroy/clear. " +
            "For 'kill' type this must be a CHARACTER prefab. " +
        ...
        aiGroupPrefab: z
          ...
          .optional()
          .describe(
            "(objective) Prefab path for the AI group to spawn via SlotAI " +
            "(e.g. '{GUID}Prefabs/Groups/OPFOR/Group_USSR_LightFireTeam.et'). " +
        ...
        triggerRadius: z
          .number()
          .default(100)
          .describe("(objective) Radius in metres of the Area trigger that activates the objective. Default 100."),
        faction: z
          .string()
          .optional()
        ...
        baseName: z
          .string()
          .optional()
          .describe("(base) Base name/callsign, e.g. 'BaseAlpha'. Used as entity name prefix and m_sBaseName property."),
        baseFaction: z
          .string()
          .optional()
          .describe("(base) Faction that starts as owner: 'US', 'USSR', 'FIA', or a custom faction key."),
        baseType: z
          .enum(["base", "MOB"])
          .optional()
          .describe(
            "(base) Base type. 'base' (default) = standard contested base. " +
        ...
        patrolCount: z
          ...
          .max(6)
          .default(2)
          .describe("(base) Number of ambient patrol spawnpoints to place around the base (0-6, default 2)."),
      },
    },
    async (args) => {
      const { type } = args;
      ...
      if (type === "objective") {
        ...
        if (!taskType || !taskName || !description || !targetPrefab || !aiGroupPrefab) {
          return {
            content: [{
              type: "text" as const,
              text: "**scenario_create (objective) failed**\nRequired params for type='objective': taskType, taskName, description, targetPrefab, aiGroupPrefab.",
            }],
            isError: true,
          };
        }
        ...
        const posResult = await resolvePosition(client, position, "scenario_create");
        ...
        try {
          ...
          return {
            content: [{
              type: "text" as const,
              text: [
                `**scenario_create (objective) failed**`,
                `Error: ${msg}`,
                ``,
                cleaned.length > 0
      ...
      if (type === "base") {
        ...
        if (!baseName || !baseFaction) {
          return {
            content: [{
              type: "text" as const,
              text: "**scenario_create (base) failed**\nRequired params for type='base': baseName, baseFaction.",
            }],
            isError: true,
          };
        }
        ...
        try {
          ...
          await setEntityProp(client, propWarnings, names.base, 'SCR_FactionAffiliationComponent."faction affiliation"', baseFaction);
          if (baseType === "MOB") {
          ...
          const patrolPrefab = PATROL_PREFAB_BY_FACTION[baseFaction] ?? CONFLICT_PATROL_PREFAB_DEFAULT;
          const needsFactionProp = !(baseFaction in PATROL_PREFAB_BY_FACTION);
          ...
          for (let i = 0; i < count; i++) {
            ...
            if (needsFactionProp) {
              await setEntityProp(client, propWarnings, patrolName, 'SCR_FactionAffiliationComponent."faction affiliation"', baseFaction);
            }
          }
          ...
          await setEntityProp(client, propWarnings, names.spawnPoint, "m_sFaction", baseFaction);
          ...
          lines.push(
            ...
            baseType !== "MOB" ? `5. Ensure at least one MOB per faction exists in the world for radio coverage.` : `5. MOB placed — verify radio coverage reaches contested bases.`,
          );
      ...
      // Should never reach here due to z.enum constraint, but TypeScript needs a return
      return {
        content: [{ type: "text" as const, text: `**scenario_create failed**\nUnknown type.` }],
        isError: true,
      };
    }
  );
