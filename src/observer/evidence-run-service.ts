...
export class EvidenceRunService implements CaptureRunPort {
  ...
  async reserve(input: RunCaptureReservation): Promise<ReservedRunCapture> {
    const result = object(await this.agent.request("runReserveCapture", {
      ...
      ...(input.request.expectedWorldRevision !== undefined ? { expectedWorldRevision: input.request.expectedWorldRevision } : {}),
      ...
      ...(input.request.expectedWorldId !== undefined ? { expectedWorldId: input.request.expectedWorldId } : {}),
      ...(input.request.expectedWorldEpoch !== undefined ? { expectedWorldEpoch: input.request.expectedWorldEpoch } : {}),
