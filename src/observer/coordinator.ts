...
export interface ObserverCoordinatorOptions {
  ...
  workbenchAdapter?: Pick<
    WorkbenchObserverAdapter,
    "instances" | "submit" | "status" | "cancel" | "release" | "readCompletedArtifact" | "restoreAll"
  >;
...
export class ObserverCoordinator {
  ...
  private readonly workbenchJobs = new Map<string, { sessionId?: string; instanceId: string }>();
  ...
  async capture(input: ObserverCaptureInput): Promise<ObserverCaptureResult> {
    ...
    if (this.workbenchIdempotency.has(workbenchIdempotencyKey)) {
      return this.captureWorkbench(input, timeoutMs, workbenchIdempotencyKey);
    }
    ...
    return this.captureRuntime(input as ObserverCaptureInput & { sessionId: string }, timeoutMs);
  }
  ...
  private async captureWorkbench(
    ...
    if (!retained) {
      retained = {
        fingerprint,
        submission: adapter.submit({ view: input.view, settlePolls: input.settleFrames ?? 0 }),
      };
    ...
    this.workbenchJobs.set(status.jobId, { sessionId: input.sessionId, instanceId: status.instanceId });
    ...
    if (completed.image.length === 0 || completed.image.length > this.maxInlineImageBytes) {
      throw new ObserverCoordinatorError("ARTIFACT_TOO_LARGE", "Validated Workbench PNG exceeds the MCP inline image limit", { job });
    }
  ...
  async jobStatus(sessionId: string | undefined, jobId: string): Promise<Record<string, unknown>> {
    ...
    if (workbench) {
      ...
      try {
        const status = await this.workbenchAdapter!.status(jobId);
  ...
  async releaseJob(sessionId: string | undefined, jobId: string): Promise<Record<string, unknown>> {
    ...
    if (workbench) {
      this.assertWorkbenchJobSession(workbench.sessionId, sessionId);
      try {
        const released = await this.workbenchAdapter!.release(jobId);
        const result = { backend: "workbench", ...released };
        this.rememberWorkbenchRelease(jobId, workbench.sessionId, result);
        return result;
      } catch (error) {
        throw this.mapWorkbenchError(error);
      }
